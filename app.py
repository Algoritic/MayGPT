...
AZURE_COSMOSDB_ENABLE_FEEDBACK = os.environ.get(
    "AZURE_COSMOSDB_ENABLE_FEEDBACK", "false").lower() == "true"
...
ELASTICSEARCH_STRICTNESS = os.environ.get("ELASTICSEARCH_STRICTNESS",
                                          SEARCH_STRICTNESS)
...
frontend_settings = {
    ...
    and AZURE_COSMOSDB_DATABASE not in [None, ""],
}
...
def is_chat_model():
    ...
    ) or AZURE_OPENAI_MODEL_NAME.lower() in [
...
def prepare_body_headers_with_data(request):
    ...
    body = {
        ...
        request_messages,
        "temperature":
        float(AZURE_OPENAI_TEMPERATURE),
        "max_tokens":
        int(AZURE_OPENAI_MAX_TOKENS),
        "top_p":
        float(AZURE_OPENAI_TOP_P),
        "stop":
    ...
    if DATASOURCE_TYPE == "AzureCognitiveSearch":
        ...
        elif AZURE_SEARCH_USE_SEMANTIC_SEARCH.lower(
        ) == "true" and AZURE_SEARCH_SEMANTIC_SEARCH_CONFIG:
            query_type = "semantic"
        ...
        body["dataSources"].append({
            ...
            "parameters": {
                ...
                "fieldsMapping": {
                    ...
                    if AZURE_SEARCH_CONTENT_COLUMNS else [],
                    "titleField":
                    AZURE_SEARCH_TITLE_COLUMN
                    if AZURE_SEARCH_TITLE_COLUMN else None,
                    "urlField":
                    AZURE_SEARCH_URL_COLUMN
                    if AZURE_SEARCH_URL_COLUMN else None,
                    "filepathField":
                    AZURE_SEARCH_FILENAME_COLUMN
                    if AZURE_SEARCH_FILENAME_COLUMN else None,
                    "vectorFields":
                    parse_multi_columns(AZURE_SEARCH_VECTOR_COLUMNS)
                    if AZURE_SEARCH_VECTOR_COLUMNS else []
                },
                "inScope":
                True
                if AZURE_SEARCH_ENABLE_IN_DOMAIN.lower() == "true" else False,
                "topNDocuments":
                ...
                if AZURE_SEARCH_SEMANTIC_SEARCH_CONFIG else "",
                "roleInformation":
                AZURE_OPENAI_SYSTEM_MESSAGE,
                "filter":
                filter,
        ...
        body["dataSources"].append({
            ...
            "parameters": {
                ...
                "fieldsMapping": {
                    ...
                    parse_multi_columns(
                        AZURE_COSMOSDB_MONGO_VCORE_CONTENT_COLUMNS)
                    if AZURE_COSMOSDB_MONGO_VCORE_CONTENT_COLUMNS else [],
                    "titleField":
                    AZURE_COSMOSDB_MONGO_VCORE_TITLE_COLUMN
                    if AZURE_COSMOSDB_MONGO_VCORE_TITLE_COLUMN else None,
                    "urlField":
                    AZURE_COSMOSDB_MONGO_VCORE_URL_COLUMN
                    if AZURE_COSMOSDB_MONGO_VCORE_URL_COLUMN else None,
                    "filepathField":
                    AZURE_COSMOSDB_MONGO_VCORE_FILENAME_COLUMN
                    if AZURE_COSMOSDB_MONGO_VCORE_FILENAME_COLUMN else None,
                    "vectorFields":
                    parse_multi_columns(
                        AZURE_COSMOSDB_MONGO_VCORE_VECTOR_COLUMNS)
                    if AZURE_COSMOSDB_MONGO_VCORE_VECTOR_COLUMNS else []
                },
                "inScope":
                True if AZURE_COSMOSDB_MONGO_VCORE_ENABLE_IN_DOMAIN.lower()
                == "true" else False,
                "topNDocuments":
            ...
            "dataSources": [{
                ...
                "parameters": {
                    ...
                    "fieldsMapping": {
                        ...
                        if ELASTICSEARCH_CONTENT_COLUMNS else [],
                        "titleField":
                        ELASTICSEARCH_TITLE_COLUMN
                        if ELASTICSEARCH_TITLE_COLUMN else None,
                        "urlField":
                        ELASTICSEARCH_URL_COLUMN
                        if ELASTICSEARCH_URL_COLUMN else None,
                        "filepathField":
                        ELASTICSEARCH_FILENAME_COLUMN
                        if ELASTICSEARCH_FILENAME_COLUMN else None,
                        "vectorFields":
                        parse_multi_columns(ELASTICSEARCH_VECTOR_COLUMNS)
                        if ELASTICSEARCH_VECTOR_COLUMNS else []
                    },
                    "inScope":
                    True if ELASTICSEARCH_ENABLE_IN_DOMAIN.lower() == "true"
                    else False,
                    "topNDocuments":
    ...
    if DEBUG_LOGGING:
        ...
        if body_clean["dataSources"][0]["parameters"].get("connectionString"):
            body_clean["dataSources"][0]["parameters"][
                "connectionString"] = "*****"
...
def stream_with_data(body, headers, endpoint, history_metadata={}):
    ...
    try:
        with s.post(endpoint, json=body, headers=headers, stream=True) as r:
            for line in r.iter_lines(chunk_size=10):
                ...
                if line:
                    ...
                    response["apim-request-id"] = r.headers.get(
                        'apim-request-id')
                    ...
                    if role == "tool":
                        ...
                        deltaText = lineJson["choices"][0]["messages"][0][
                            "delta"]["content"]
                        if deltaText != "[DONE]":
                            response["choices"][0]["messages"].append({
                                "role":
        ...
        logging.exception(
            f"Exception in stream_with_data: {traceback.format_exc()}")
...
def formatApiResponseNoStreaming(rawResponse):
    ...
    toolMessage = {
        ...
        "tool",
        "content":
        rawResponse["choices"][0]["message"]["context"]["messages"][0]
        ["content"]
    }
    assistantMessage = {
        "role": "assistant",
    ...
    if rawResponse["choices"][0]["delta"].get("context"):
        messageObj = {
            "delta": {
                ...
                "tool",
                "content":
                rawResponse["choices"][0]["delta"]["context"]["messages"][0]
                ["content"]
            }
        }
...
def conversation_without_data(request_body):
    ...
    if not SHOULD_STREAM:
        ...
        return Response(stream_without_data(response, history_metadata),
                        mimetype='text/event-stream')
...
def conversation_internal(request_body):
    try:
        ...
        logging.exception(
            f"Exception in /conversation: {traceback.format_exc()}")
        return jsonify(
...
def add_conversation():
    ...
    try:
        ...
        logging.exception(
            f"Exception in /history/generate: {traceback.format_exc()}")
        return jsonify(
            {"error": "Something went wrong, please try again later"}), 500
...
def update_conversation():
    ...
    try:
        ...
        logging.exception(
            f"Exception in /history/update: {traceback.format_exc()}")
        return jsonify(
            {"error": "Something went wrong, please try again later"}), 500
...
def update_message():
    ...
    try:
        ...
        updated_message = cosmos_conversation_client.update_message_feedback(
            user_id, message_id, message_feedback)
        if updated_message:
            return jsonify({
                "message":
                f"Successfully updated message with feedback {message_feedback}",
                "message_id": message_id
            }), 200
        else:
            return jsonify({
                "error":
                f"Unable to update message {message_id}. It either does not exist or the user does not have access to it."
            }), 404

    except Exception as e:
        logging.exception(
            f"Exception in /history/message_feedback: {traceback.format_exc()}"
        )
        return jsonify(
            {"error": "Something went wrong, please try again later"}), 500
...
def delete_conversation():
    ...
    try:
        ...
        logging.exception(
            f"Exception in /history/delete: {traceback.format_exc()}")
        return jsonify(
            {"error": "Something went wrong, please try again later"}), 500
...
def list_conversations():
    ...
    if not isinstance(conversations, list):
        return jsonify({"error":
                        f"No conversations for {user_id} were found"}), 404
...
def delete_all_conversations():
    ...
    try:
        ...
        if not conversations:
            return jsonify(
                {"error": f"No conversations for {user_id} were found"}), 404
        ...
        return jsonify({
            "message":
            f"Successfully deleted conversation and messages for user {user_id}"
        }), 200

    except Exception as e:
        logging.exception(
            f"Exception in /history/delete_all: {traceback.format_exc()}")
        return jsonify(
            {"error": "Something went wrong, please try again later"}), 500
...
def clear_messages():
    ...
    try:
        ...
        logging.exception(
            f"Exception in /history/clear_messages: {traceback.format_exc()}")
        return jsonify(
            {"error": "Something went wrong, please try again later"}), 500
...
def get_frontend_settings():
    try:
        ...
        logging.exception(
            f"Exception in /frontend_settings: {traceback.format_exc()}")
        return jsonify(
            {"error": "Something went wrong, please try again later"}), 500
...
def generate_title(conversation_messages):
    ...
    try:
        ...
        openai.api_key = AZURE_OPENAI_KEY
        completion = openai.ChatCompletion.create(engine=AZURE_OPENAI_MODEL,
                                                  messages=messages,
                                                  temperature=1,
                                                  max_tokens=64)
        title = json.loads(
            completion['choices'][0]['message']['content'])['title']
        return title
    except Exception as e:
